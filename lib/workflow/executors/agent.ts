...
export async function executeAgentNode(
  ...
  try {
    ...
    if (apiKeys?.groq) availableProviders.push('groq');

    if (!availableProviders.includes(provider)) {
      if (availableProviders.length > 0) {
        const fallbackProvider = availableProviders[0];
        console.warn(`⚠️ Provider '${provider}' not available, falling back to '${fallbackProvider}'`);
        provider = fallbackProvider;
        if (fallbackProvider === 'anthropic' && !modelName.includes('claude')) modelName = 'claude-opus-4-1';
        else if (fallbackProvider === 'openai' && !modelName.includes('gpt')) modelName = 'gpt-4o-mini';
        else if (fallbackProvider === 'groq' && !modelName.includes('mixtral')) modelName = 'mixtral-8x7b-32768';
      } else {
        throw new Error('❌ No LLM API keys configured. Please add ANTHROPIC_API_KEY, OPENAI_API_KEY, or GROQ_API_KEY to .env.local');
      }
    }
    ...
    if (provider === 'anthropic' && apiKeys?.anthropic) {
      ...
      throw new Error(`❌ Provider '${provider}' is not configured. Available providers: ${['anthropic', 'openai', 'groq'].filter(p => 
        (p === 'anthropic' && apiKeys?.anthropic) || 
        (p === 'openai' && apiKeys?.openai) || 
        (p === 'groq' && apiKeys?.groq)
      ).join(', ') || 'None - please add API keys to .env.local'}`);
    }
