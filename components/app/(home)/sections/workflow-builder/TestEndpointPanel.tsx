...
with requests.post(
  ...
  return (
    <motion.aside
      ...
      className="fixed right-20 top-80 h-[calc(100vh-100px)] w-[calc(100vw-240px)] max-w-520 bg-card-bg border border-border-faint shadow-lg overflow-y-auto z-50 rounded-16 flex flex-col"
    >
      {/* Header */}
      <div className="p-20 border-b border-border-faint flex-shrink-0">
        <div className="flex items-center justify-between">
          <h2 className="text-label-large text-text-primary font-medium">Endpoint</h2>
          <button
            onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-20 space-y-20">
        ...
        <div>
          ...
          <div className="px-12 py-10 bg-background-base border border-border-faint rounded-8 text-body-small text-text-primary font-mono overflow-x-auto">
            {endpointUrl}
          </div>
        </div>
            ...
            return (
              <div>
                ...
                <div className="p-12 bg-heat-4 rounded-8 border border-heat-100">
                  ...
                  <pre className="text-body-small text-text-primary font-mono whitespace-pre-wrap overflow-auto max-h-200">
          ...
          {!hasRealKey && (
            <div className="mb-12 p-12 bg-heat-4 border border-heat-100 rounded-8">
              <p className="text-body-small text-text-primary">
                <strong>Note:</strong> Replace <code className="px-4 py-2 bg-white/10 rounded text-xs font-mono">YOUR_API_KEY_HERE</code> with your actual API key from Settings.
              </p>
            </div>
          )}
            ...
            <TabsContent value="curl">
              <div className="relative">
                <button
                  ...
                  className="absolute top-12 right-12 flex items-center gap-6 px-12 py-6 bg-card-bg hover:bg-[#f4f4f5] border border-border-faint rounded-8 text-xs text-text-primary transition-colors shadow-sm"
                >
                  {copiedKey === 'curl' ? 'Copied' : 'Copy'}
                </button>
                <pre className="px-12 py-10 bg-background-base text-text-primary rounded-8 text-body-small font-mono whitespace-pre-wrap break-words overflow-y-auto overflow-x-hidden max-h-200 border border-border-faint">
{curlStandard}
                </pre>
              </div>
            </TabsContent>
...
{pythonExample}
        ...
        {/* Response */}
        {error && (
          <div className="p-16 bg-white/5 rounded-12 border border-border-faint">
            <h3 className="text-label-small text-text-primary mb-8">Error</h3>
            <pre className="text-body-small text-text-primary whitespace-pre-wrap">
              {error}
            </pre>
          </div>
        )}
