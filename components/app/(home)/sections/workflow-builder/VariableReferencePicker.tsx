        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="fixed w-400 max-w-[calc(100vw-40px)] bg-card-bg border border-border-faint rounded-12 shadow-2xl z-[9999] overflow-hidden"
              style={{
            ...
            >
              <div className="p-12 border-b border-border-faint">
                <h4 className="text-label-small text-text-primary">Available Variables</h4>
              </div>
              ...
              {variables.map((group, groupIndex) => (
                <div key={groupIndex}>
                  <div className="px-12 py-8 bg-background-base">
                    <p className="text-body-small text-text-secondary font-medium">
                      {group.category}
                    </p>
                  </div>
                      ...
                      <div className="flex items-start justify-between gap-8">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className={`text-body-small font-medium break-all ${
                            item.isField || item.isInputVariable || item.isNested ? 'text-heat-100' : 'text-text-primary'
                          }`}>
                            {item.name}
                          </p>
                          <p className="text-body-small text-text-secondary mt-2 font-mono text-[10px]">
                            {`{{${item.path}}}`}
                          </p>
                          {item.description && (
                            <p className="text-body-small text-text-secondary mt-4 truncate">
                              {item.description}
                            </p>
                          )}
                        </div>
                        <div className="flex flex-col gap-4 items-end">
                          {item.propertyType && (
                            <span className="px-6 py-2 bg-white/5 text-text-primary rounded-4 text-[10px] font-medium flex-shrink-0">
                              {item.propertyType}
                            </span>
                          )}
