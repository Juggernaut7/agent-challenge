...
export default function NodePanel({
  ...
  const getAvailableModels = () => {
    ...
    // Always add Nosana Qwen for ElizaForge
    models.push({
      provider: 'Nosana (GPU)',
      models: [
        { id: 'nosana/qwen-3.5-27b', name: 'Qwen 3.5 (27B)' },
      ]
    });
    ...
    <AnimatePresence>
      {nodeData && (
        <motion.aside
          ...
          className="fixed right-20 top-64 h-[calc(100vh-84px)] w-[calc(100vw-240px)] max-w-480 bg-card-bg border border-border-system shadow-2xl overflow-y-auto z-50 rounded-12 custom-scrollbar"
        >
          {/* Header */}
          <div className="p-16 border-b border-border-system bg-white/5">
            <div className="flex items-center justify-between mb-8">
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="text-label-large font-bold text-text-primary bg-transparent border-none outline-none focus:outline-none hover:bg-white/5 px-4 -ml-4 rounded-4 transition-colors"
                placeholder="Enter node identity..."
              />
              <div className="flex items-center gap-4">
                <button className="w-28 h-28 rounded-6 hover:bg-white/10 transition-colors flex items-center justify-center text-text-muted hover:text-text-primary">
                  <svg
                    className="w-14 h-14"
                    fill="none"
                    stroke="currentColor"
                ...
                <button
                  ...
                  className="w-28 h-28 rounded-6 hover:bg-red-500/10 transition-colors flex items-center justify-center group text-text-muted hover:text-red-500"
                  title="Delete node"
                >
                  <svg
                    className="w-14 h-14"
                ...
                <button
                  onClick={onClose}
                  className="w-28 h-28 rounded-6 hover:bg-white/10 transition-colors flex items-center justify-center text-text-muted hover:text-text-primary"
                >
                  <svg
                    className="w-14 h-14"
                    fill="none"
            ...
            <p className="text-label-x-small font-bold text-primary-green uppercase tracking-wider">
              {nodeData.type} Configuration
            </p>
          </div>
          ...
          <div className="p-20 space-y-20">
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                <label className="block text-label-small font-medium text-text-secondary">
                  Instructions
                </label>
              ...
              <textarea
                ...
                rows={8}
                className="w-full px-14 py-10 bg-card-bg border border-border-system rounded-10 text-label-medium text-text-primary focus:outline-none focus:border-primary-green transition-colors resize-y"
              />
            ...
            {/* Specialized Fields for Elizaforge Nodes */}
            {nodeData?.type === 'research' && (
              <div>
                <label className="block text-label-small font-medium text-text-secondary mb-8">
                  Search Query
                </label>
                <input
                  type="text"
                  value={(nodeData as any).searchQuery || ""}
                  onChange={(e) => onUpdate(nodeData.id, { searchQuery: e.target.value })}
                  placeholder="What to research?"
                  className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm focus:outline-none focus:border-heat-100"
                />
              </div>
            )}

            {nodeData?.type === 'summarizer' && (
              <div>
                <label className="block text-label-small font-medium text-text-secondary mb-8">
                  Summary Length
                </label>
                <select
                  value={(nodeData as any).summaryLength || "medium"}
                  onChange={(e) => onUpdate(nodeData.id, { summaryLength: e.target.value })}
                  className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm focus:outline-none focus:border-heat-100"
                >
                  <option value="short">Short (Bullet points)</option>
                  <option value="medium">Medium (Detailed summary)</option>
                  <option value="long">Long (Full report)</option>
                </select>
              </div>
            )}

            {nodeData?.type === 'social-post' && (
              <div className="space-y-12">
                <div>
                  ...
                  <select
                    value={(nodeData as any).socialPlatform || "twitter"}
                    onChange={(e) => onUpdate(nodeData.id, { socialPlatform: e.target.value })}
                    className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm focus:outline-none focus:border-heat-100"
                  >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={(nodeData as any).postTemplate || ""}
                    onChange={(e) => onUpdate(nodeData.id, { postTemplate: e.target.value })}
                    placeholder="e.g. Professional, Viral, Shitpost"
                    className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm focus:outline-none focus:border-heat-100"
                  />
                </div>
              </div>
            ...
            <div className="flex items-center justify-between py-8">
              <label className="text-label-small font-medium text-text-primary">
            ...
            <div>
              ...
              <button
                ...
                className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm text-text-primary focus:outline-none focus:border-heat-100 transition-colors flex items-center justify-between hover:bg-white/5"
              >
                ...
                <ChevronDown
                  className={`w-16 h-16 text-text-secondary transition-transform ${
              ...
              {showModelsDropdown && (
                <div className="mt-8 p-8 bg-card-bg border border-border-system rounded-10 space-y-8 max-h-[300px] overflow-y-auto custom-scrollbar">
                  {getAvailableModels().length === 0 ? (
                    <div className="p-16 text-center">
                      ...
                      <button
                        ...
                        className="px-12 py-6 bg-primary-green text-white rounded-8 text-label-small hover:bg-hover-green transition-colors"
                      >
                        Add API Keys
                      </button>
                    </div>
              ...
              {model === "custom" && (
                <input
                  ...
                  className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm text-text-primary placeholder-black-alpha-32 font-mono focus:outline-none focus:border-heat-100 transition-colors mt-8"
                />
              )}
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-8">
                  ...
                  >
                    <svg
                      className="w-18 h-18 text-text-secondary"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                >
                  <div className="flex items-center justify-between mb-12">
                    ...
                    <button
                      ...
                      className="w-20 h-20 rounded-6 hover:bg-white/5 transition-colors flex items-center justify-center"
                    >
                      <svg
                  ...
                  <div className="space-y-12">
                    {!mcpServers || mcpServers.length === 0 ? (
                      ...
                      mcpServers.map((server: any) => {
                        ...
                        return (
                          <div key={server._id} className="rounded-12 border border-border-faint overflow-hidden bg-card-bg">
                            <button
                              onClick={() => setExpandedMcpId(isExpanded ? null : server._id)}
                              className="w-full px-16 py-12 flex items-center justify-between text-left hover:bg-white/5 transition-colors"
                            >
                              <div className="flex items-center gap-8">
                                <span className="text-label-small font-medium text-text-primary">{server.name}</span>
                                {isFirecrawl && (
                                ...
                                {server.tools && (
                                  <span className="px-6 py-2 bg-background-base text-text-secondary rounded-6 text-xs border border-border-faint">
                                    {server.tools.length} tools
                                  </span>
                                )}
                              </div>
                              <svg
                                className={`w-16 h-16 text-text-muted transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                            ...
                            {isExpanded && (
                              <div className="px-16 pb-16 space-y-10 bg-card-bg border-t border-border-faint">
                                {server.description && (
                                  <p className="pt-12 text-xs text-text-primary">{server.description}</p>
                                )}
                                {isFirecrawl && (
                                  <a
                                ...
                                {server.tools && server.tools.length > 0 && (
                                  <div className="space-y-6">
                                    <p className="text-xs text-text-primary font-medium">Available Tools:</p>
                                    <div className="flex flex-wrap gap-4">
                                      {server.tools.map((tool: string) => (
                                        <span key={tool} className="px-6 py-2 bg-background-base text-text-primary rounded-4 text-xs border border-border-faint">
                                          {tool}
                                        </span>
                                      ))}
                                    </div>
                                ...
                                <div className="flex items-center justify-end pt-8">
                                  <button
                                    ...
                                    className={`px-12 py-8 rounded-8 text-xs font-medium transition-colors ${
                                      isConnected
                                        ? 'bg-card-bg border border-border-faint text-text-primary hover:bg-white/5'
                    ...
                    <div className="rounded-12 border border-border-faint overflow-hidden bg-card-bg">
                      <button
                        onClick={() => setExpandedMcpId(expandedMcpId === 'custom' ? null : 'custom')}
                        className="w-full px-16 py-12 flex items-center justify-between text-left hover:bg-white/5 transition-colors"
                      >
                        <div className="flex items-center gap-8">
                          <span className="text-label-small font-medium text-text-primary">Add New MCP Server</span>
                          <span className="px-6 py-2 bg-heat-4 text-heat-100 rounded-6 text-xs border border-heat-100">
                            Settings
                          </span>
                        </div>
                        <svg
                          className={`w-16 h-16 text-text-muted transition-transform ${expandedMcpId === 'custom' ? 'rotate-180' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="w-20 h-20 rounded-6 hover:bg-white/5 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100"
                        >
                          <svg
                            className="w-14 h-14 text-text-secondary hover:text-text-primary"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                          >
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-14 py-10 bg-background-base border border-border-faint rounded-10 text-sm text-text-primary focus:outline-none focus:border-heat-100 transition-colors appearance-none cursor-pointer"
              >
                      ...
                      {schemaFields.map((field, index) => (
                        ...
                        >
                          <input
                            ...
                            className="flex-1 px-10 py-6 bg-card-bg border border-border-faint rounded-6 text-sm text-text-primary placeholder-black-alpha-32 focus:outline-none focus:border-heat-100"
                          />
                          <select
                            value={field.type}
                            onChange={(e) => {
                            ...
                            className="px-10 py-6 bg-card-bg border border-border-faint rounded-6 text-sm text-text-primary focus:outline-none focus:border-heat-100"
                          >
                          ...
                          <button
                            ...
                            className="w-24 h-24 rounded-6 hover:bg-white/5 transition-colors flex items-center justify-center"
                          >
                            <svg
                              className="w-14 h-14 text-text-secondary"
                              fill="none"
            ...
            <details className="group" open={showAdvanced}>
              <summary
                ...
                className="flex items-center justify-between cursor-pointer list-none text-label-small font-medium text-text-secondary hover:text-text-primary transition-colors py-12"
              >
                <span>Advanced</span>
                <svg
