...
export default function SettingsPanel({ isOpen, onClose }: SettingsPanelProps) {
  ...
  const StatusIcon = ({ configured }: { configured: boolean }) =>
    configured ? (
      ...
      <XCircle className="w-16 h-16 text-text-muted" />
    );
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-card-bg rounded-16 shadow-2xl max-w-2xl w-full mx-20 flex flex-col"
        ...
        >
          ...
          <div className="p-24 border-b border-border-faint flex-shrink-0">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-title-h4 text-text-primary">Configuration Status</h2>
              ...
              <button
                onClick={onClose}
                className="w-32 h-32 rounded-8 hover:bg-white/5 transition-colors flex items-center justify-center"
              >
          ...
          <div className="p-24 overflow-y-auto flex-1">
            {loading ? (
              <div className="text-center py-32">
                ...
                <p className="text-body-small text-text-secondary mt-12">Loading configuration...</p>
              </div>
            ) : (
              <div className="space-y-20">
                ...
                <div>
                  <div className="flex items-center justify-between mb-12">
                    <h3 className="text-label-large font-medium text-text-primary">LLM Providers</h3>
                    <button
                        ...
                        <div key={provider} className="p-12 bg-background-base rounded-8 border border-border-faint">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-8">
                              ...
                              <div>
                                <p className="text-body-small text-text-primary font-medium capitalize">{provider}</p>
                                {providerKey ? (
                                  <p className="text-xs text-text-secondary">
                                    Key: {providerKey.keyPrefix} {providerKey.label && `(${providerKey.label})`}
                                  </p>
                                ) : hasEnvKey ? (
                                  <p className="text-xs text-text-secondary">Using environment variable</p>
                                ) : (
                            ...
                            <div className="flex items-center gap-8">
                              {providerKey && (
                                <>
                                  <button
                                    ...
                                    className="p-6 hover:bg-white/5 rounded-6 transition-colors"
                                    title="Remove key"
                                  >
                                    <Trash2 className="w-14 h-14 text-text-secondary hover:text-text-primary" />
                                  </button>
                                </>
                              )}
                              <button
                              ...
                              >
                                {providerKey ? (
                                  <Edit className="w-14 h-14 text-text-secondary hover:text-text-primary" />
                                ) : (
                                  <Plus className="w-14 h-14 text-text-secondary hover:text-text-primary" />
                                )}
                              </button>
                            </div>
                          </div>
                ...
                {/* Integrations */}
                <div>
                  <h3 className="text-label-large font-medium text-text-primary mb-12">Integrations</h3>
                  <div className="grid grid-cols-1 gap-8">
                    <div className="flex items-center gap-8 p-10 bg-background-base rounded-8 border border-border-faint">
                      ...
                      <div className="min-w-0">
                        <p className="text-body-small text-text-primary font-medium truncate">Firecrawl</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* API Keys */}
                <div>
                  <h3 className="text-label-large font-medium text-text-primary mb-12">Your API Keys</h3>
                    ...
                    <div className="p-16 bg-heat-4 border border-heat-100 rounded-8 mb-12">
                      <div className="flex items-start gap-12 mb-12">
                        ...
                        <div className="flex-1">
                          <p className="text-body-medium text-text-primary font-medium mb-4">
                            Save this key now!
                          </p>
                          <p className="text-body-small text-text-primary mb-8">
                            You won't be able to see it again.
                          </p>
                          <div className="flex items-center gap-8">
                            <code className="flex-1 px-12 py-8 bg-card-bg text-text-primary border-border-system rounded-8 text-xs font-mono text-text-primary">
                              {generatedKey}
                            </code>
                            <button
                              onClick={() => {
                              ...
                              className="px-12 py-8 bg-accent-black hover:bg-white/108 text-white rounded-8 text-body-small font-medium transition-all active:scale-[0.98] flex items-center gap-6"
                            >
                      ...
                      <button
                        ...
                        className="text-body-small text-text-primary hover:text-text-primary"
                      >
                        I've saved it
                      </button>
                    </div>
                  ...
                  <div className="flex gap-8 mb-12">
                    <input
                      ...
                      className="flex-1 px-12 py-8 bg-background-base border border-border-faint rounded-8 text-body-small text-text-primary placeholder:text-text-muted"
                    />
                    <button
                      onClick={async () => {
                        if (!newKeyName.trim()) {
                    ...
                    {apiKeys?.map((key) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <p className="text-body-small text-text-primary font-medium">{key.name}</p>
                          <code className="text-xs text-text-secondary font-mono">{key.keyPrefix}</code>
                        ...
                        <button
                          ...
                          className="p-8 hover:bg-white/5 rounded-8 transition-colors"
                          title="Revoke key"
                        >
                          <Trash2 className="w-16 h-16 text-text-secondary hover:text-text-primary" />
                        </button>
                      </div>
                    ))}

                    {(!apiKeys || apiKeys.length === 0) && !generatedKey && (
                      <p className="text-body-small text-text-secondary text-center py-16">
                ...
                <div>
                  <div className="flex items-center justify-between mb-12">
                    <h3 className="text-label-large font-medium text-text-primary">MCP Registry</h3>
                    <div className="flex gap-8">
                      <button
                        onClick={() => setShowPasteConfigModal(true)}
                        className="px-12 py-6 bg-white/5 hover:bg-white/10 text-text-primary rounded-8 text-body-small font-medium transition-all active:scale-[0.98] flex items-center gap-6 border border-border-faint"
                      >
                  ...
                  <div className="space-y-8">
                    ...
                    {(!mcpServers || mcpServers.length === 0) && (
                      <div className="text-center py-32">
                        <Plug className="w-48 h-48 text-text-muted mx-auto mb-12" />
                        <p className="text-body-small text-text-secondary mb-16">
                  ...
                  <div className="flex items-start gap-12">
                    ...
                    <div>
                      ...
                      <p className="text-body-small text-text-primary">
                        LLM & Integration keys are set in <code className="px-4 py-2 bg-white/10 rounded text-xs font-mono">.env.local</code>.
          ...
          <div className="p-20 border-t border-border-faint bg-background-base rounded-b-16 flex-shrink-0">
            <button
              onClick={onClose}
              className="w-full px-20 py-12 bg-accent-black hover:bg-white/108 text-white rounded-8 text-body-medium font-medium transition-all active:scale-[0.98]"
            >
              Close
            </button>
          </div>
...
function MCPCard({
  ...
  const getStatusColor = () => {
    switch (server.connectionStatus) {
      ...
      case 'error': return 'text-text-primary';
  ...
  const getAuthIcon = () => {
    switch (server.authType) {
      case 'api-key': return <Key className="w-12 h-12 text-text-secondary" />;
      case 'bearer': return <Shield className="w-12 h-12 text-text-secondary" />;
      case 'oauth-coming-soon': return <Lock className="w-12 h-12 text-text-muted" />;
      default: return null;
    }
  };
    ...
    <div className={`bg-background-base rounded-8 border ${server.enabled ? 'border-border-faint' : 'border-black-alpha-8 opacity-60'}`}>
      ...
      <div className="p-12 flex items-center gap-8">
        <button
          ...
          className="p-4 hover:bg-white/5 rounded-4 transition-colors"
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-8">
            <p className="text-body-small text-text-primary font-medium">{server.name}</p>
            {server.isOfficial && (
          ...
          {server.description && (
            <p className="text-xs text-text-secondary mt-2">{server.description}</p>
          )}
        ...
        <div className="flex items-center gap-8">
          <label className="relative inline-flex items-center cursor-pointer">
            ...
            />
            <div className="w-36 h-20 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-16 after:w-16 after:transition-all peer-checked:bg-primary-green"></div>
          </label>
        </div>
      </div>
        ...
        <div className="px-12 pb-12 border-t border-border-faint">
          <div className="pt-12 space-y-12">
            ...
            <div className="grid grid-cols-2 gap-12">
              <div>
                <p className="text-xs text-text-secondary mb-4">URL</p>
                <code className="text-xs font-mono text-text-primary bg-white/5 px-8 py-4 rounded-4 block truncate">
                  {server.url}
                </code>
              </div>
              <div>
                <p className="text-xs text-text-secondary mb-4">Authentication</p>
                <div className="flex items-center gap-4">
                  ...
                  {server.accessToken && <span className="text-xs text-text-muted">•••••••</span>}
                </div>
              </div>
            </div>
            ...
            {server.tools && server.tools.length > 0 && (
              <div>
                <p className="text-xs text-text-secondary mb-4">Available Tools ({server.tools.length})</p>
                <div className="flex flex-wrap gap-4">
                  {server.tools.map((tool) => (
                    <span key={tool} className="px-8 py-4 bg-white/5 text-xs text-text-primary rounded-4">
                      {tool}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-8 pt-8">
              <button
                onClick={onTest}
                disabled={isTesting}
                className="px-12 py-6 bg-white/5 hover:bg-white/10 text-text-primary rounded-8 text-body-small transition-all flex items-center gap-6 disabled:opacity-50"
              >
              ...
              <button
                onClick={onEdit}
                className="px-12 py-6 bg-white/5 hover:bg-white/10 text-text-primary rounded-8 text-body-small transition-all flex items-center gap-6"
              >
              ...
              {!server.isOfficial && (
                <button
                  onClick={onDelete}
                  className="px-12 py-6 bg-white/5 hover:bg-accent-black hover:text-white text-text-secondary rounded-8 text-body-small transition-all flex items-center gap-6"
                >
  ...
  return (
    ...
    >
      <motion.div
        initial={{ scale: 0.95 }}
        animate={{ scale: 1 }}
        className="bg-card-bg rounded-16 shadow-2xl max-w-md w-full mx-20 max-h-[85vh] overflow-hidden flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-24 border-b border-border-faint flex-shrink-0">
          <h3 className="text-title-h4 text-text-primary">
        ...
        <div className="p-24 space-y-16 overflow-y-auto flex-1">
          <div>
            <label className="text-body-small text-text-primary mb-4 block">Name</label>
            <input
              type="text"
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              placeholder="e.g., Firecrawl"
              className="w-full px-12 py-8 bg-background-base border border-border-faint rounded-8 text-body-small text-text-primary"
            />
          </div>

          <div>
            <label className="text-body-small text-text-primary mb-4 block">URL</label>
            <input
              type="text"
              value={formData.url}
              onChange={(e) => setFormData({ ...formData, url: e.target.value })}
              placeholder="https://mcp.example.com"
              className="w-full px-12 py-8 bg-background-base border border-border-faint rounded-8 text-body-small text-text-primary font-mono"
            />
          </div>

          <div>
            <label className="text-body-small text-text-primary mb-4 block">Description (optional)</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-body-small text-text-primary mb-4 block">Category</label>
            <select
          ...
          <div>
            <label className="text-body-small text-text-primary mb-4 block">Authentication</label>
            <select
          ...
          {(formData.authType === 'api-key' || formData.authType === 'bearer') && (
            <div>
              <label className="text-body-small text-text-primary mb-4 block">
          ...
          <div>
            <button
              ...
              className="w-full px-16 py-10 bg-white/5 hover:bg-white/10 text-text-primary rounded-8 text-body-small font-medium transition-all flex items-center justify-center gap-6 disabled:opacity-50"
            >
              {isTesting ? (
                <>
              ...
              <div className="p-12 bg-heat-4 rounded-8 border border-heat-100">
                <div className="flex flex-wrap gap-4">
                  {discoveredTools.map((tool) => (
                    <span key={tool} className="px-6 py-2 bg-primary-green/10 text-accent-green rounded-4 text-xs font-medium border border-primary-green/20">
                      {tool}
                    </span>
                  ))}
                </div>
        ...
        <div className="p-20 border-t border-border-faint flex gap-8 flex-shrink-0">
          <button
            onClick={onClose}
            className="flex-1 px-20 py-12 bg-white/5 hover:bg-white/10 text-text-primary rounded-8 text-body-medium font-medium transition-all"
          >
            Cancel
          </button>
          <button
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-24 space-y-16 overflow-y-auto flex-1">
          <div>
            <label className="text-body-small text-text-primary mb-4 block">Provider</label>
            <select
              value={formData.provider}
              onChange={(e) => setFormData({ ...formData, provider: e.target.value as 'anthropic' | 'openai' | 'groq' })}
              disabled={!!selectedProvider}
              className="w-full px-12 py-8 bg-background-base border border-border-faint rounded-8 text-body-small text-text-primary capitalize"
            >
          ...
          <div>
            <label className="text-body-small text-text-primary mb-4 block">API Key</label>
            <div className="relative">
              <input
                ...
                className="w-full pr-32 px-12 py-8 bg-background-base border border-border-faint rounded-8 text-body-small text-text-primary font-mono"
              />
              <button
                type="button"
                onClick={() => setShowKey(!showKey)}
                className="absolute right-8 top-1/2 -translate-y-1/2 p-4 hover:bg-white/5 rounded-4 transition-colors"
              >
                {showKey ? (
                  <EyeOff className="w-16 h-16 text-text-secondary" />
                ) : (
          ...
          <div>
            <label className="text-body-small text-text-primary mb-4 block">Label (optional)</label>
            <input
              type="text"
        ...
        <div className="p-20 border-t border-border-faint flex gap-8 flex-shrink-0">
          <button
            onClick={onClose}
            disabled={isSaving}
            className="flex-1 px-20 py-12 bg-white/5 hover:bg-white/10 text-text-primary rounded-8 text-body-medium font-medium transition-all disabled:opacity-50"
          >
            Cancel
          </button>
          <button
