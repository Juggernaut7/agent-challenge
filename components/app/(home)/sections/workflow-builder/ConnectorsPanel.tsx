      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-white/58 z-[100]"
            onClick={onClose}
          />
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-screen w-full max-w-600 bg-card-bg border-l border-border-faint shadow-2xl z-[101] flex flex-col"
          >
            {/* Header */}
            <div className="p-20 border-b border-border-faint">
              <div className="flex items-center justify-between mb-12">
                <h2 className="text-title-h3 text-text-primary">Connectors & Config</h2>
                <button
                  onClick={onClose}
                ...
                <button
                  ...
                  className={`px-16 py-8 rounded-8 text-body-medium font-medium transition-all ${
                    activeTab === 'mcp'
                      ...
                      : 'bg-background-base text-text-primary hover:bg-white/5'
                  }`}
                >
            ...
            <div className="flex-1 overflow-y-auto">
              {activeTab === 'mcp' ? (
                <div>
                  ...
                  {mcpTab === 'templates' ? (
                    <div className="p-20 space-y-16">
                      <div>
                        <h3 className="text-label-medium text-text-primary mb-12">MCP Templates</h3>
                              ...
                              <div className="flex items-start justify-between mb-12">
                                <div className="flex-1">
                                  <div className="flex items-center gap-8 mb-6">
                                    <h4 className="text-label-medium text-text-primary font-medium">{template.name}</h4>
                                    <span className="px-6 py-2 bg-heat-4 text-heat-100 rounded-4 text-body-small">
                                      {template.category}
                                    </span>
                                  </div>
                                  <p className="text-body-small text-text-primary mb-8">{template.description}</p>
                                  <p className="text-body-small text-text-secondary font-mono text-xs truncate">
                    ...
                    <div className="p-20 space-y-20">
                      <div>
                        <h3 className="text-label-medium text-text-primary mb-12">Add Custom MCP Server</h3>

                        <div className="space-y-16">
                          <div>
                            <label className="block text-label-small text-text-secondary mb-8">Name</label>
                            <input
                              type="text"
                              value={customName}
                              onChange={(e) => setCustomName(e.target.value)}
                              placeholder="My Custom MCP"
                              className="w-full px-12 py-10 bg-card-bg border border-border-faint rounded-8 text-body-medium text-text-primary focus:outline-none focus:border-heat-100 transition-colors"
                            />
                          </div>

                          <div>
                            <label className="block text-label-small text-text-secondary mb-8">URL</label>
                            <input
                              type="text"
                              value={customUrl}
                              onChange={(e) => setCustomUrl(e.target.value)}
                              placeholder="https://mcp.example.com"
                              className="w-full px-12 py-10 bg-card-bg border border-border-faint rounded-8 text-body-medium text-text-primary font-mono focus:outline-none focus:border-heat-100 transition-colors"
                            />
                          </div>

                          <div>
                            <label className="block text-label-small text-text-secondary mb-8">Authentication</label>
                            <select
                  ...
                  {connectedMCPs.length > 0 && (
                    <div className="p-20 border-t border-border-faint">
                      <h3 className="text-label-medium text-text-primary mb-12">Connected MCPs ({connectedMCPs.length})</h3>
                      <div className="space-y-12">
                        {connectedMCPs.map((mcp) => (
                          <div key={mcp.id} className="p-12 bg-card-bg rounded-8 border border-border-faint">
                            <div className="flex items-center justify-between">
                              <div className="flex-1">
                                <p className="text-body-small text-text-primary font-medium">{mcp.name}</p>
                                <p className="text-body-small text-text-secondary font-mono text-xs truncate">{mcp.url}</p>
                              </div>
                              <button
                                onClick={() => handleDisconnect(mcp.id)}
                                className="px-12 py-6 bg-background-base hover:bg-white/5 border border-border-faint hover:border-border-faint rounded-6 text-body-small text-text-primary transition-colors"
                              >
                                Disconnect
                              </button>
                            </div>
                ...
                <div className="p-20">
                  <h3 className="text-label-medium text-text-primary mb-16">LLM Configuration</h3>
                  <div className="p-16 bg-card-bg rounded-12 border border-border-faint text-center">
