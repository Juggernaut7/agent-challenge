    ...
    <AnimatePresence>
      {(node || mode === 'add-to-agent') && (
        <motion.aside
          ...
          className="fixed right-20 top-80 bottom-20 w-[calc(100vw-240px)] max-w-520 bg-card-bg border border-border-faint shadow-lg overflow-y-auto z-50 rounded-16 flex flex-col"
        >
          {/* Header */}
                                ...
                                >
                                  <div className="space-y-6">
                                    {selectedServer.tools.map((tool: string) => (
                                      <div key={tool} className="p-8 bg-card-bg rounded-6 border border-border-faint">
