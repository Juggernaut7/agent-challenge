  ...
  return (
    <AnimatePresence>
      <motion.aside
        ...
        className="fixed right-20 top-80 h-[calc(100vh-100px)] w-480 bg-card-bg border border-border-faint shadow-lg overflow-y-auto z-50 rounded-16"
      >
        {/* Header */}
        <div className="p-20 border-b border-border-faint sticky top-0 bg-card-bg">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-title-h3 text-text-primary">Workflow Preview</h2>
            <button
              onClick={onClose}
          ...
          {execution && (
            <div className={`inline-flex items-center gap-8 px-12 py-6 rounded-8 text-body-small ${
              ...
              execution.status === 'failed' ? 'bg-white/5 text-text-primary' :
        ...
        <div className="p-20">
          {isRunning && Object.keys(nodeResults).length === 0 ? (
            ...
            <div className="text-center py-32">
              <svg className="w-48 h-48 mx-auto mb-16 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-body-medium text-text-secondary">Click Preview to run workflow</p>
            </div>
          ) : (
              ...
              {Object.entries(nodeResults).map(([nodeId, result]) => (
                <div key={nodeId} className="bg-background-base rounded-12 p-16 border border-border-faint">
                  ...
                  <div className="flex items-center justify-between mb-12">
                    <h3 className="text-label-medium text-text-primary font-medium">
                      {nodeId}
                    </h3>
                    <span className={`text-body-small px-8 py-4 rounded-6 ${
                      ...
                      result.status === 'failed' ? 'bg-white/5 text-text-primary' :
                      'bg-gray-50 text-gray-600'
                    }`}>
                      {result.status}
                    </span>
                  ...
                  {result.output && (
                    <div className="mt-12">
                      <p className="text-body-small text-text-secondary mb-8">Output:</p>
                      <div className="bg-card-bg rounded-8 p-12 border border-border-faint">
                        <pre className="text-body-small text-text-primary whitespace-pre-wrap overflow-auto max-h-200">
                  ...
                  {result.error && (
                    <div className="mt-12">
                      <p className="text-body-small text-text-primary mb-8">Error:</p>
                  ...
                  {result.startedAt && result.completedAt && (
                    <p className="text-body-small text-text-muted mt-8">
