  ...
  return (
    <AnimatePresence>
      <motion.aside
        ...
        className="fixed right-20 top-80 h-[calc(100vh-100px)] w-[calc(100vw-240px)] max-w-480 bg-card-bg border border-border-faint shadow-lg overflow-hidden z-50 rounded-16 flex flex-col"
      >
        {/* Header */}
        <div className="p-20 border-b border-border-faint flex-shrink-0">
          <div className="flex items-center justify-between">
            <h2 className="text-title-h3 text-text-primary">Extract (Schema)</h2>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-20 space-y-24">
          ...
          <div>
            ...
            />
            <p className="text-body-small text-text-muted mt-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows={12}
              className={`w-full px-12 py-10 bg-background-base border rounded-8 text-body-small text-text-primary font-mono focus:outline-none focus:border-heat-100 transition-colors resize-none ${
                schemaError ? 'border-red-500' : 'border-border-faint'
              }`}
            />
            {schemaError && (
              <p className="text-body-small text-text-primary mt-6">{schemaError}</p>
            )}
            <p className="text-body-small text-text-muted mt-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                onClick={onAddMCP}
                className="px-10 py-6 bg-background-base hover:bg-white/5 border border-border-faint rounded-6 text-body-small text-text-primary transition-colors flex items-center gap-6"
              >
                  ...
                  <div key={index} className="p-12 bg-background-base rounded-8 border border-border-faint">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-body-small text-text-secondary font-mono text-xs truncate mt-4">
                          {mcp.url}
                        </p>
                      </div>
                      <button
          ...
          {/* Info Box */}
          <div className="p-16 bg-card-bg rounded-12 border border-border-faint">
