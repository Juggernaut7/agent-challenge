...
const Menu = ({ setOpen }: { setOpen: (open: boolean) => void }) => {
  ...
  return (
    <motion.div
      ...
      className="absolute w-220 whitespace-nowrap rounded-16 p-4 bg-card-bg left-0 top-[calc(100%+8px)] z-[2000] border border-border-system"
      exit={{ opacity: 0, y: 8, scale: 0.98, filter: "blur(1px)" }}
      initial={{ opacity: 0, y: -6, filter: "blur(1px)" }}
      style={{
        boxShadow:
          "0px 12px 24px rgba(0, 0, 0, 0.4), 0px 4px 8px rgba(0, 0, 0, 0.2)",
      }}
      transition={{
        ease: cubicBezier(0.1, 0.1, 0.25, 1),
        duration: 0.2,
    ...
    >
      <div
        className="absolute top-4 opacity-0 z-[2] pointer-events-none inset-x-4 bg-white/5 rounded-8 h-32"
        ref={backgroundRef}
      />
      ...
      <Button
        onClick={() => {
          window.open("https://github.com/Nosana-CI/nosana-node", "_blank");
          setOpen(false);
        }}
        onMouseEnter={onMouseEnter}
        onMouseLeave={onMouseLeave}
...
const Button = (attributes: React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  return (
    <button
      {...attributes}
      className={cn(
        "flex gap-8 w-full items-center text-label-small group text-text-primary p-6 hover:text-accent-green transition-colors",
