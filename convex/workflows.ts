...
export const list = query({
  ...
  handler: async (ctx) => {
    ...
    // Default: return all non-template workflows if unauthenticated
    // This unblocks the user while keeping templates separate
    return await ctx.db
      .query("workflows")
...
export const saveWorkflow = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const newId = await ctx.db.insert("workflows", {
      ...args,
      userId: identity?.subject || "public_user", // Fallback to public if no auth
