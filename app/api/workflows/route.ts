...
export async function GET(request: NextRequest) {
  try {
    ...
    const isAuthError = errorMessage.includes('NoAuthProvider') || 
                       errorMessage.includes('auth provider found matching');
    
    if (isAuthError) {
      return NextResponse.json(
        {
          error: 'Authentication configuration issue',
          message: 'JWT token missing "aud" (audience) claim',
          workflows: [],
          fix: 'See CLERK_JWT_FIX.md for instructions',
        },
        { status: 401 }
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isAuthError) {
      // This is specifically the Clerk JWT audience claim issue
      return NextResponse.json(
        {
          error: 'Authentication configuration issue',
          message: 'JWT token missing "aud" (audience) claim',
          fix: {
            step1: 'Go to Clerk Dashboard: https://dashboard.clerk.com',
            step2: 'Navigate to JWT Templates and click "convex" template',
            step3: 'Add custom claim: {"aud": "convex"}',
            step4: 'Save and return to ElizaForge to try again',
            documentation: 'See CLERK_JWT_FIX.md for detailed instructions',
          },
          debugInfo: {
            errorCode: 'NoAuthProvider',
            possibleCause: 'Clerk JWT template missing audience claim',
            token_check: 'Run: Check browser DevTools → Network → auth-debug endpoint',
          },
        },
        { status: 401 }
      );
    ...
    return NextResponse.json(
      {
        ...
        message: errorMessage.substring(0, 200), // Truncate long error messages
      },
      { status: 500 }
    );
  }
